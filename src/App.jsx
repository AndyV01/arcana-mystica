          ...
          {[{icon:"👁",val:stats.visits,label:t.seekers},{icon:"🔮",val:stats.readings,label:t.readings}].map(({icon,val,label}) => (
            <div key={label} className="stat-pill" style={{ display:"flex",alignItems:"center",gap:"6px",padding:"4px 12px",background:"rgba(255,255,255,.03)",border:"1px solid rgba(140,80,255,.15)",borderRadius:"20px",cursor:"default",transition:"all .3s" }}>
              ...
              <div>
                ...
                <div style={{ fontSize:"7px",letterSpacing:"1.5px",color:"rgba(180,140,255,.3)",textTransform:"uppercase" }}>{label}</div>
              </div>
            </div>
          ))}
        ...
        {tab === "home" && phase === "idle" && (
          <div style={{ maxWidth:"920px",margin:"0 auto",padding:"40px 20px 20px",textAlign:"center" }}>
            <div style={{ opacity:visible.header?1:0,animation:visible.header?"headerIn .9s cubic-bezier(.34,1.56,.64,1) forwards":"none" }}>
              <div style={{ fontSize:"52px",marginBottom:"14px",animation:"textGlow 3s ease-in-out infinite",display:"inline-block" }}>🔮</div>
              ...
              <h1 className="logo-text" style={{ fontSize:"clamp(36px,9vw,72px)",fontFamily:"'Cinzel',serif",fontWeight:400,lineHeight:1.08,marginBottom:"18px",background:"linear-gradient(135deg,#f0e2ff 0%,#c8a4ff 40%,#9060ef 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",letterSpacing:"2px" }}>
                {t.appName}
              </h1>
