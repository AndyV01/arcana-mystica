...
const T = {
  en: {
    ...
    dayPH: "DD", yearPH: "YYYY",
    lifePath: "Life Path", zodiac: "Zodiac Sign",
    personalNote: "Your reading has been attuned to your cosmic blueprint",
    months: ["January","February","March","April","May","June","July","August","September","October","November","December"],
    errors: { day: "Enter a day between 1 and 31", month: "Select a month", year: "Enter a valid year (1900–2025)", future: "You cannot be born in the future!" },
  },
  es: {
    ...
    dayPH: "DD", yearPH: "AAAA",
    lifePath: "Camino de Vida", zodiac: "Signo Zodiacal",
    personalNote: "Tu lectura ha sido sintonizada con tu mapa cósmico",
    months: ["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"],
    errors: { day: "Ingresa un día entre 1 y 31", month: "Selecciona un mes", year: "Ingresa un año válido (1900–2025)", future: "¡No puedes haber nacido en el futuro!" },
  },
}
...
export default function BirthDateModal({ lang, spreadName, onConfirm, onSkip }) {
  ...
  const d = parseInt(day), m = parseInt(month), y = parseInt(year)
  const lifePathNum = (day && month && year && !Object.keys(errors).length) ? calcNumerology(d, m, y) : null
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: "linear-gradient(135deg,rgba(22,8,45,0.92),rgba(12,4,25,0.95))",
        ...
        <p style={{ fontSize:"9px",letterSpacing:"5px",color:"rgba(200,160,255,.45)",textTransform:"uppercase",fontFamily:"'Cinzel','Georgia',serif",marginBottom:"10px" }}>✦ {spreadName} ✦</p>
        <h2 style={{ fontSize:"clamp(20px,4vw,28px)",fontFamily:"'Cinzel','Georgia',serif",fontWeight:400,color:"#ede0ff",marginBottom:"10px",letterSpacing:"1px" }}>{t.title}</h2>
        <p style={{ fontSize:"13px",color:"rgba(190,160,240,.55)",fontFamily:"'Cormorant Garamond','Georgia',serif",fontStyle:"italic",marginBottom:"32px",lineHeight:1.6 }}>{t.subtitle}</p>
        <p style={{ fontSize:"11px",color:"rgba(210,185,245,.58)",fontFamily:"'Cormorant Garamond','Georgia',serif",fontStyle:"italic",marginTop:"-18px",marginBottom:"24px",lineHeight:1.5 }}>{t.dailyNote}</p>
        ...
        <div style={{ display:"flex",gap:"12px",marginBottom:"8px",justifyContent:"center" }}>
          <div style={{ flex:"0 0 72px" }}>
            ...
            <input ref={dayRef} type="number" min="1" max="31" placeholder={t.dayPH} value={day}
          ...
          <div style={{ flex:"1" }}>
            ...
            <select id="bd-month" value={month}
              ...
              <option value="">—</option>
              {t.months.map((mn, i) => <option key={i} value={i + 1}>{mn}</option>)}
            </select>
          ...
          <div style={{ flex:"0 0 90px" }}>
            ...
            <input ref={yearRef} type="number" min="1900" max="2025" placeholder={t.yearPH} value={year}
        ...
        {zodiac && lifePathNum && (
          <div style={{ display:"flex",gap:"10px",justifyContent:"center",marginBottom:"24px",marginTop:"18px",opacity:revealed?1:0.35,transform:revealed?"scale(1)":"scale(0.95)",transition:"all 0.55s cubic-bezier(.34,1.56,.64,1)" }}>
            <div className="num-card" style={{ flex:1,padding:"14px 10px",background:"linear-gradient(135deg,rgba(100,50,200,.25),rgba(60,20,120,.2))",border:"1px solid rgba(160,100,255,.22)",borderRadius:"12px" }}>
              ...
              <div style={{ fontSize:"10px",color:"rgba(200,170,240,.6)",marginTop:"6px",fontFamily:"'Cormorant Garamond','Georgia',serif",fontStyle:"italic",lineHeight:1.4 }}>{getLifePathMeaning(lifePathNum, lang)}</div>
            </div>
            <div className="num-card" style={{ flex:1,padding:"14px 10px",background:"linear-gradient(135deg,rgba(60,20,120,.25),rgba(100,50,200,.2))",border:"1px solid rgba(160,100,255,.22)",borderRadius:"12px" }}>
              ...
              <div style={{ fontSize:"12px",color:"#d4b4ff",marginTop:"6px",fontFamily:"'Cinzel','Georgia',serif",letterSpacing:"1px" }}>{zodiac?.[lang]}</div>
            </div>
          </div>
        )}

        {!revealed ? (
          <button className="bd-cta" onClick={handlePreview} style={{ width:"100%",padding:"15px",background:"linear-gradient(135deg,rgba(120,60,200,.35),rgba(80,30,160,.35))",border:"1px solid rgba(180,120,255,.4)",borderRadius:"10px",color:"#e8d5ff",fontSize:"12px",letterSpacing:"3px",textTransform:"uppercase",cursor:"pointer",fontFamily:"'Cinzel','Georgia',serif",transition:"all .35s",marginBottom:"14px" }}>
            ✦ {t.cta} ✦
          </button>
        ) : (
          <button className="bd-cta" onClick={handleConfirm} style={{ width:"100%",padding:"15px",background:"linear-gradient(135deg,rgba(140,70,220,.5),rgba(90,40,180,.5))",border:"1px solid rgba(200,150,255,.5)",borderRadius:"10px",color:"#f0e4ff",fontSize:"12px",letterSpacing:"3px",textTransform:"uppercase",cursor:"pointer",fontFamily:"'Cinzel','Georgia',serif",transition:"all .35s",marginBottom:"14px",boxShadow:"0 0 30px rgba(160,100,255,.3)" }}>
            🔮 {t.cta} 🔮
          </button>
        )}

        <button className="bd-skip" onClick={onSkip} style={{ background:"none",border:"none",color:"rgba(180,140,255,.3)",fontSize:"11px",letterSpacing:"1.5px",cursor:"pointer",fontFamily:"'Cormorant Garamond','Georgia',serif",fontStyle:"italic",transition:"color .3s" }}>
          {t.skip}
        </button>
      </div>
    </div>
